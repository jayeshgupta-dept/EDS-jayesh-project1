...
async function handleHover(pathname) {
  const hoverContainerPath = pathname;
  ...
  return updatedSubNav;
}

// async function generateSubNav(pathname, hoverInnerDiv) {
//   // 1) Load fragment
//   const subNav = await loadFragment(pathname);
//   const subNavString = subNav instanceof Element ? subNav.outerHTML : subNav;

//   // 2) Replace <main> with <div>
//   let updatedSubNav = subNavString
//     .replace('<main>', '<div>')
//     .replace('</main>', '</div>');

//   // 3) Work in a temp container
//   const tempDiv = document.createElement('div');
//   tempDiv.innerHTML = updatedSubNav;

//   // 4) Set all <li> active="false"
//   tempDiv.querySelectorAll('li').forEach(li => li.setAttribute('active', 'false'));

//   // 5) Merge top-level ULs inside categories
//   const categoryBlock = tempDiv.querySelector('.categories.block');
//   if (categoryBlock) {
//     // Find all top-level <ul> elements under first .categories.block
...
//     // 6) Add right-arrow icons on top-level button-container
//     const topCategoryParas = categoryBlock.querySelectorAll(
//       ':scope > div > div > ul > li > p.button-container'
//     );

//     topCategoryParas.forEach(p => {
//       const a = p.querySelector('a');
//       if (a && !p.querySelector('.icon-right-arrow')) {
...
//   // 7) Inject transformed HTML into hover container
//   updatedSubNav = tempDiv.innerHTML;
//   hoverInnerDiv.innerHTML = updatedSubNav;

//   // 8) Re-init category hover behaviour (second column + down arrows)
...
//     img.src = '/icons/right-arrow.svg'; // same as your original
//     img.alt = '';
//     img.loading = 'lazy';
...
//     // 🖱 Hover on left-side category
//     item.addEventListener('mouseenter', () => {
//       console.log('Theeeeeeeeeese run on hover', item);

//       // Use your existing helper to clear active
//       removeArrowFromAll();

//       // Fill second column with cloned UL
//       secondDiv.innerHTML = '';
//       if (storing) {
...
//       // Attach activeInside hover for items in second column
//       setTimeout(() => {
...
export default async function decorate(block) {
  ...
  // const fragmentssssssss = await loadFragment(navPath);
  const subNav = await loadFragment(hoverContainerPath);
  const subNavString = subNav instanceof Element ? subNav.outerHTML : subNav;
  ...
  // console.log("airConditionerLink", airConditionerLink);
  ...
  if (main) {
    main.querySelectorAll('.section').forEach((section) => {
      ...
      // console.log(" newWrapper?.remove();", newWrapper);
  ...
  // ================= Dynamic Hover Loading Without Cache ======================
  // document.querySelectorAll('.header-new-wrapper > div > div > div > ul > li')
  //   .forEach((item, index) => {
  //     const isLinkAvalilable = item.getAttribute('sub-nav');

  //     item.addEventListener('mouseenter', async () => {
  //       // if (isLinkAvalilable) {
  //       //   const KKKKKKKKKKKKK =  await handleHover(`/` + isLinkAvalilable);
  //       //   console.log("KKK", KKKKKKKKKKKKK);
  //       // }
  ...
  //       if (isLinkAvalilable) {
  //       try {
  //         const fetchedHTML = await handleHover(`/${isLinkAvalilable}`);
  //         const subNavContent = fetchedHTML;

  //         const hoverInnerDiv = hoverContainer.querySelector('div');
  //         hoverInnerDiv.innerHTML = subNavContent;

  //         console.log(`✅ Dynamic sub-nav loaded for: ${isLinkAvalilable}`);
  //       } catch (error) {
  //         console.error(`❌ Failed to load sub-nav for ${isLinkAvalilable}`, error);
  //       }
  //     }
  //     });
  ...
  //     });
  //   });
  // ================= Dynamic Hover Loading With Cache ======================

  // Cache object to store already-fetched subnav HTML fragments
  /**
   * htmlString: raw fragment from handleHover()
   * activeSlug: the slug for the hovered header item (optional) — used to set top-level active="true"
   */
  function transformSubNav(htmlString, activeSlug = '') {
    // console.log("htmlstring11111111111111111", htmlString);

    const temp = document.createElement('div');
    // replace main -> div as before
    temp.innerHTML = (htmlString || '')
      .replace('<main>', '<div>')
      .replace('</main>', '</div>');

    // 1) set active="false" on all li initially
  ...
  headerItems.forEach((item, index) => {
    item.addEventListener('mouseenter', async () => {
      ...
      // 1. Load slug HTML
      // const rawHTML = await handleHover(`/${slug}`);
      // const finalHTML = transformSubNav(rawHTML);
      // const myHtml = generateSubNav(`/${slug}`, hoverInnerDiv);
      await generateSubNav(`/fragment/${slug}`, hoverInnerDiv, slug);
      // const slot = hoverContainer.querySelector(':scope > div > div');

      // if (!slot) {
      //   console.warn("❌ ERROR: Could not find .sub-nav-container > div > div");
      //   return;
      // }
      // 4. Inject transformed HTML
      // slot.innerHTML = finalHTML;
      // 5. Show hover styling
      removeApplyBorderFromAll();
      arrowDragger(index);
      item.classList.add('applyBorderOnHover');
      toggleExpanded(true);
  ...
  // 🔴 OLD category hover logic (now handled by initCategoryHover)
  // Keeping for reference, but commented out to avoid double-binding
  // document.querySelectorAll('.categories.block > div > div > ul > li').forEach(item => {
  //   const secondDiv = document.querySelector('.categories.block > div:nth-child(2)');
  //   const storing = item.querySelector('ul');

  //   const addIcon = storing.querySelectorAll('li > p.button-container');
  //   // console.log(secondDiv, "asbhas", addIcon);
  //   const span = document.createElement('span');
  //   span.className = 'icon icon-down-arrow';
  //   const img = document.createElement('img');
  //   img.dataset.iconName = 'down-arrow';
  ...
  //   img.loading = 'lazy';
  //   span.appendChild(img);
  //   addIcon.forEach(container => container.appendChild(span.cloneNode(true)));
  //   item.addEventListener('mouseenter', () => {
  //     console.log("Theeeeeeeeeese run on hover", item);
  //     removeArrowFromAll();
  //     secondDiv.innerHTML = '';
  ...
  //     item.setAttribute('active', 'true');
  //     setTimeout(() => {
  //       document.querySelectorAll(".categories.block>div:nth-child(2) li").forEach(items => {
  //         items.setAttribute('activeInside', 'false'); //check this
  //         // console.log("asbashybasibsa", items.getAttribute('active'));
  //         items.addEventListener('mouseenter', () => {
  //           console.log("Icamebu");
  //           // removeDownArrowFromAll();
  //           items.setAttribute('activeInside', 'true');
  //         });
  //         items.addEventListener('mouseleave', () => {
  //           console.log("byee");
  //           items.setAttribute('activeInside', 'false');
  //         });
  //       });
  //     }, 200);
  //   });
  ...
  function removeDownArrowFromAll() {
    // const liItems = document.querySelectorAll('li[active="true"]');   
    // liItems.forEach(li => {
    //   li.setAttribute('active', 'false');
    const jasn = document.querySelector('.categories.block > div:nth-child(2)');
    console.log("jasnjasnjasn", jasn);
    // });
  }

  document.querySelectorAll(".header.block>div:nth-child(2)>ul:first-child").forEach(item => {
    console.log("asbashybasibsa", item);
  ...
  if (navSections) {
    navSections.querySelectorAll(':scope > ul > li').forEach((navSection, index) => {
      ...
      navSection.addEventListener('click', async () => {
        ...
        //   // 🔹 Look for an existing host *inside* this li (navSection)
        //   let host = navSection.querySelector(':scope > .dropdown-host');

        //   if (!host) {
        //     host = document.createElement('div');
        //     host.className = 'dropdown-host';

        //     // Optional: if you want the dropdown panel to be positioned
        //     // relative to the li, you can keep this as relative
        //     host.style.position = 'relative';

        //     // 🔥 Append INSIDE the li instead of after it
        //     navSection.appendChild(host);
        //   }
        ...
        //       // since host is inside navSection, this still works fine
